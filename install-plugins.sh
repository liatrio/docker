...
doDownload() {
   ...
   url="http://ftp-chi.osuosl.org/pub/jenkins/plugins/$plugin/$version/${plugin}.hpi"
    echo "Downloading plugin: $plugin from $url"
    curl --connect-timeout ${CURL_CONNECTION_TIMEOUT:-20} --retry ${CURL_RETRY:-5} --retry-delay ${CURL_RETRY_DELAY:-0} --retry-max-time ${CURL_RETRY_MAX_TIME:-60}  -f -L "$url" -o "$jpi"
    return $?
}
